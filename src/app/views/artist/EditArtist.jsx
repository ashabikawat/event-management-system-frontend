...
const EditArtist = () => {
  ...
  useEffect(() => {
    setFormValues({
      artistName: editObject.artist_name || "",
      artistDescription: editObject.artist_description || "",
      artistImages: editObject.artist_image || [],
    });

    setSelectedImagesPreview(editObject.artist_image);
  }, [editObject]);
  ...
  const getArtistByID = async () => {
    try {
      ...
      const response = await axiosWrapper(
        artistEndpoints.GET_ARTIST_BY_ID,
        payload,
        "post"
      );
      setEditObject(response.data.artist);
    } catch (error) {
      if (error) {
